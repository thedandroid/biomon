...
must_haves:
  truths:
    ...
    - "Built executable runs successfully (smoke test)"
...
5. Smoke test the built executable (pkg binary validation):
```bash
# Start server in background, verify it starts, then stop
timeout 5 node dist/biomon.cjs &
SERVER_PID=$!
sleep 2
curl -s http://localhost:3050/ > /dev/null && echo "Server responds OK" || echo "Server failed to respond"
kill $SERVER_PID 2>/dev/null || true
```

Note: This validates the pkg-built bundle runs correctly, not just that it was created.
...
8. Document final test count and any notes.
  </action>
  <verify>
npm run typecheck && npm test && npm run build
All commands must exit 0
Built executable must respond to HTTP request (smoke test)
  </verify>
  <done>All 79 tests pass, typecheck passes, build succeeds, executable runs correctly, migration complete</done>
</task>
...
- npm run build succeeds
- Built executable runs and responds to HTTP requests (smoke test)
- No any types in any test file
