...
Session handlers need additional dependencies for persistence functions. First, update `src/handlers/types.ts` to add SessionDependencies:

```typescript
// Add to src/handlers/types.ts (after HandlerDependencies interface):
import type { AutosaveInfo, CampaignInfo, SaveResult, LoadResult } from "../types/index.js";
...
Then create the session handlers file:
