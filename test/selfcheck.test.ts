...
describe("selfcheck validation", () => {
  it("should validate that all table entries have required fields", () => {
    ...
    ([...STRESS_TABLE, ...PANIC_TABLE] as TableEntry[]).forEach((entry: TableEntry) => {
      requiredFields.forEach((field) => {
        expect(entry).toHaveProperty(field);
        expect(entry[field as keyof TableEntry]).toBeDefined();
      });
    });
  });
  ...
  it("should validate table ranges don't overlap incorrectly", () => {
    [STRESS_TABLE, PANIC_TABLE].forEach((table) => {
      table.forEach((entry: TableEntry) => {
  ...
  it("should validate all IDs are unique within each table", () => {
    const stressIds: string[] = STRESS_TABLE.map((e: TableEntry) => e.id);
    const panicIds: string[] = PANIC_TABLE.map((e: TableEntry) => e.id);
