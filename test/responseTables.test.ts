...
describe("responseTables", () => {
  describe("STRESS_TABLE", () => {
    it("should have valid table structure", () => {
      ...
      STRESS_TABLE.forEach((entry: TableEntry) => {
    ...
    it("should have no gaps in coverage", () => {
      ...
      for (let total = -10; total <= 30; total++) {
        const entry: TableEntry = resolveEntry("stress", total);
  ...
  describe("PANIC_TABLE", () => {
    it("should have valid table structure", () => {
      ...
      PANIC_TABLE.forEach((entry: TableEntry) => {
        expect(entry).toHaveProperty("min");
        expect(entry).toHaveProperty("max");
        expect(entry).toHaveProperty("id");
        expect(entry).toHaveProperty("label");
    ...
    it("should have no gaps in coverage", () => {
      for (let total = -10; total <= 30; total++) {
        const entry: TableEntry = resolveEntry("panic", total);
        expect(entry).toBeDefined();
        expect(entry.id).toBeTruthy();
      }
    });
  ...
  describe("resolveEntry", () => {
    it("should return 'Keeping Cool' for stress total <= 0", () => {
      const entry: TableEntry = resolveEntry("stress", 0);
    ...
    it("should return 'Keeping Cool' for panic total <= 0", () => {
      const entry: TableEntry = resolveEntry("panic", 0);
      expect(entry.id).toBe("panic_keeping_cool");
      expect(entry.label).toBe("Keeping Cool");
    });

    it("should return 'Jumpy' for stress total = 1", () => {
      const entry: TableEntry = resolveEntry("stress", 1);
    ...
    it("should return 'Spooked' for panic total = 1", () => {
      const entry: TableEntry = resolveEntry("panic", 1);
    ...
    it("should handle negative totals", () => {
      const stressEntry: TableEntry = resolveEntry("stress", -5);
      const panicEntry: TableEntry = resolveEntry("panic", -5);
    ...
    it("should default to stress table for unknown roll type", () => {
      const entry: TableEntry = resolveEntry("unknown", 1);
      expect(entry.id).toBe("stress_jumpy");
    });
  });
  ...
  describe("getEntryById", () => {
    it("should retrieve stress entry by id", () => {
      const entry: TableEntry | null = getEntryById("stress", "stress_jumpy");
      expect(entry).toBeDefined();
      expect(entry!.id).toBe("stress_jumpy");
      expect(entry!.label).toBe("Jumpy");
    });
    ...
    it("should retrieve panic entry by id", () => {
      const entry: TableEntry | null = getEntryById("panic", "panic_spooked");
      expect(entry).toBeDefined();
      expect(entry!.id).toBe("panic_spooked");
      expect(entry!.label).toBe("Spooked");
    });
    ...
    it("should return null for non-existent id", () => {
      const entry: TableEntry | null = getEntryById("stress", "non_existent_id");
      expect(entry).toBeNull();
    });

    it("should return null for undefined id", () => {
      const entry: TableEntry | null = getEntryById("stress", undefined as unknown as string);
      expect(entry).toBeNull();
    });

    it("should return null for null id", () => {
      const entry: TableEntry | null = getEntryById("stress", null as unknown as string);
      expect(entry).toBeNull();
    });

    it("should handle empty string id", () => {
      const entry: TableEntry | null = getEntryById("stress", "");
      expect(entry).toBeNull();
    });
  });
  ...
  describe("Stress table entries", () => {
    ...
    it("should have correct stress deltas", () => {
      const messUp: TableEntry | null = getEntryById("stress", "stress_mess_up");
  ...
  describe("Panic table entries", () => {
    ...
    it("should have apply options for Seek Cover", () => {
      const seekCover: TableEntry | null = getEntryById("panic", "panic_seek_cover");
    ...
    it("should have apply options for Flee", () => {
      const flee: TableEntry | null = getEntryById("panic", "panic_flee");
