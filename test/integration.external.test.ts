...
describe("External Integration via Socket.io", () => {
    ...
    await new Promise<void>((resolve) => {
      server.listen(0, () => {
        ...
        io.on("connection", (socket) => {
          ...
          socket.on("player:add", (payload: PlayerAddPayload) => {
          ...
          socket.on("player:update", (payload: PlayerUpdatePayload & { activeEffects?: Effect[] }) => {
  ...
  describe("CORS Configuration", () => {
    it("allows external client connection from allowed origin", async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        externalClient.on("connect_error", (error: Error) => {
      ...
      await new Promise<void>((resolve) => {
        externalClient.on("state", (receivedState: GameState) => {
          ...
          if (stateUpdateCount === 1) {
            ...
            const gmClient: TypedMainClient = trackClient(Client(serverUrl, { reconnection: false }));
            gmClient.on("connect", () => {
        ...
        externalClient.on("state", (receivedState: GameState) => {
          ...
          if (stateUpdateCount === 1) {
            ...
            gmClient.on("connect", () => {
              ...
              } as PlayerUpdatePayload & { activeEffects: Effect[] });
            });
  ...
  describe("Multiple External Clients", () => {
    it("supports multiple simultaneous external connections", async () => {
      const client1: TypedExternalClient = trackClient(Client(`${serverUrl}/external`, { reconnection: false }));
      const client2: TypedExternalClient = trackClient(Client(`${serverUrl}/external`, { reconnection: false }));
